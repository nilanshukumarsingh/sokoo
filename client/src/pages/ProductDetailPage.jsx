...
const ProductDetailPage = () => {
    ...
    const fetchRelatedProducts = async (category) => {
        try {
            ...
            const response = await productsAPI.getAll({ category: category, limit: 8 });
    ...
    const handleWishlist = async () => {
        ...
        try {
            ...
            showToast(updatedWishlist.includes(product._id) ? 'Added to wishlist' : 'Removed from wishlist', 'success');
                                ...
                                {product.images.map((img, idx) => (
                                    <div 
                                        ...
                                        style={{
                                            aspectRatio: '1',
                                            cursor: 'pointer',
                                            border: selectedImage === img ? '2px solid var(--fg)' : '2px solid transparent',
                                        ...
                                        onMouseLeave={(e) => e.currentTarget.style.opacity = selectedImage === img ? 1 : 0.6}
                                    >
                                        <img 
                                            src={getImageUrl(img)} 
                                            alt={`View ${idx + 1}`}
                        ...
                        {product?.stock > 0 && (
                            <div style={{ display: 'flex', gap: '1rem', alignItems: 'center', flexWrap: 'wrap' }}>
                                ...
                                <button
                                    ...
                                    onMouseEnter={(e) => !addingToCart && (e.currentTarget.style.transform = 'translateY(-2px)')}
                            ...
                            {relatedProducts.map(rp => (
                                ...
                                >
                                    <div style={{
                                        ...
                                        <img 
                                            src={rp.images?.[0] ? 
                                                (rp.images[0].startsWith('http') ? rp.images[0] : `http://localhost:5000${rp.images[0]}`) 
